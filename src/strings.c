...
int to_radix40(unsigned char c) {
  if (c == 0) return 0;
...
symbol_t pack40(const char *buffer, size_t n) {
  if (n < 4)
    fatal("too small buffer to pack.");
  return ((to_radix40(buffer[0])*40) + to_radix40(buffer[1]))*40 + to_radix40(buffer[2]);
}

void unpack40(symbol_t symbol, char *buffer, size_t n) {
  if (n < 4)
    fatal("too small buffer to unpack.");
  buffer[3] = 0;
  for (int n=2; n >= 0; n--) {
    buffer[n] = from_radix40(symbol % 40);
    symbol = symbol / 40;
  }
}
